...
class PConvUNet(nn.Module):
    def __init__(self, layer_size=7, input_channels=3, upsampling_mode='nearest'):
        ...
        #print("encoder shapes:",self.enc_1,self.enc_2.shape,self.enc_3,self.enc_4.shape)
    ...
    def forward(self, input, input_mask):
        ...
        if debug ==1:
            print(" input in forward",input.shape,input_mask.shape)
        h_dict['h_0'], h_mask_dict['h_0'] = input, input_mask

        if debug==1:
            print("tensor shapes",h_dict['h_0'].shape,h_mask_dict['h_0'].shape)
        h_key_prev = 'h_0'
        for i in range(1, self.layer_size + 1):
            ...
            h_key_prev = h_key
            if debug==1:
                print(h_key,"tensor shapes",h_dict[h_key].shape,h_mask_dict[h_key].shape)

        h_key = 'h_{:d}'.format(self.layer_size)
        if debug==1:
            print("h key is",h_key,h_dict[h_key].shape)
        h= h_dict[h_key]
        if debug==1:
            print("h key is",h_key,h_mask_dict[h_key].shape)
        h_mask=h_mask_dict[h_key]
        if debug==1:
            print("tensor shapes finally",h.shape,h_mask.shape)
        ...
        for i in range(self.layer_size, 0, -1):
            ...
            if debug==1:
                print("comes later",h.shape,h_dict[enc_h_key].shape)
