...
mask_indices = np.nonzero(final_mask)
maskVector = []
for i in range(len(mask_indices[0])):
	maskVector.append([mask_indices[1][i], mask_indices[0][i]])
plt.show(plt.imshow(np.stack([explored_map, frontiers_indicator, abs(final_mask - 1)],axis=-1)))
pdb.set_trace()
...
for artifact in _artifact_locations:
	rect = patches.Rectangle((artifact[0] - 0.5, artifact[1] - 0.5), 1, 1, linewidth=1.5, linestyle='--', joinstyle='round', edgecolor='black', hatch='', facecolor='white')
...
##################### Plot mask ########
for m in maskVector:
	rect = patches.Rectangle((m[0] - 0.5, m[1] - 0.5), 1, 1, linewidth=2, edgecolor='mediumorchid', hatch='/', facecolor='yellow')
	ax.add_patch(rect)
